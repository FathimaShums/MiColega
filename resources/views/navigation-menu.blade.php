<nav x-data="{ open: false }" class="bg-purple-600 border-b border-purple-500">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard')" class="text-black font-bold">
                {{ __('Dashboard') }}
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            ...
            <div class="mt-3 space-y-1">
                <!-- Account Management -->
                <x-responsive-nav-link href="{{ route('profile.show') }}" :active="request()->routeIs('profile.show')" class="text-white font-bold">
                    {{ __('Profile') }}
                </x-responsive-nav-link>

                @if (Laravel\Jetstream\Jetstream::hasApiFeatures())
                    <x-responsive-nav-link href="{{ route('api-tokens.index') }}" :active="request()->routeIs('api-tokens.index')" class="text-white font-bold">
                        {{ __('API Tokens') }}
                    </x-responsive-nav-link>
                @endif
                ...
                <form method="POST" action="{{ route('logout') }}" x-data>
                    ...
                    <x-responsive-nav-link href="{{ route('logout') }}"
                                   @click.prevent="$root.submit();" class="text-white font-bold">
                        {{ __('Log Out') }}
                    </x-responsive-nav-link>
                </form>
                ...
                @if (Laravel\Jetstream\Jetstream::hasTeamFeatures())
                    ...
                    <!-- Team Settings -->
                    <x-responsive-nav-link href="{{ route('teams.show', Auth::user()->currentTeam->id) }}" :active="request()->routeIs('teams.show')" class="text-white font-bold">
                        {{ __('Team Settings') }}
                    </x-responsive-nav-link>

                    @can('create', Laravel\Jetstream\Jetstream::newTeamModel())
                        <x-responsive-nav-link href="{{ route('teams.create') }}" :active="request()->routeIs('teams.create')" class="text-white font-bold">
                            {{ __('Create New Team') }}
                        </x-responsive-nav-link>
                    @endcan
